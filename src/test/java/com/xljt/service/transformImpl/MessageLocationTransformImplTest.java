package com.xljt.service.transformImpl;

import com.xljt.pojo.MessageLocationPojo;
import org.junit.Before;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.test.context.junit4.SpringRunner;

import static org.junit.Assert.*;

/**
 * @author : XU <br>
 * @version : 1.0 <br>
 * @description : MessageLocationTransformImplTest <br>
 * @date : 2019/9/16 14:21 <br>
 */
@RunWith(SpringRunner.class)
@SpringBootTest
public class MessageLocationTransformImplTest {
    String message;

    @Autowired
    private MessageLocationTransformImpl messageLocationTransform;

    @Before
    public void setUp() throws Exception {
        message = "";
    }

    @Test
    public void getMessageLocation() {
        MessageLocationPojo messageLocation = messageLocationTransform.getMessageLocation(message, 62);
        System.out.println(messageLocation);
    }
}