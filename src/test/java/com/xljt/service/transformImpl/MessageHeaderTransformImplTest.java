package com.xljt.service.transformImpl;

import com.xljt.pojo.MessageHeaderPojo;
import org.junit.Before;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.test.context.junit4.SpringRunner;

/**
 * @author : XU <br>
 * @version : 1.0 <br>
 * @description : MessageHeaderTransformTest <br>
 * @date : 2019/9/16 11:00 <br>
 */
@RunWith(SpringRunner.class)
@SpringBootTest
public class MessageHeaderTransformImplTest {

    @Autowired
    MessageHeaderTransformImpl messageHeaderTransformImpl;
    String message;
    @Before
    public void setUp() throws Exception {
        message = "";
    }

    @Test
    public void getMessage() {
        MessageHeaderPojo message = messageHeaderTransformImpl.getMessageHeader(this.message);
        System.out.println(message);
    }
}